...
if ($UploadArtifacts) {
    ...
    if ($OptionalParameters[$storageKey] -eq $null) {
        $OptionalParameters[$storageKey] = (Get-AzureRmStorageAccountKey -ResourceGroupName "ARM_Deploy_Staging" -Name $StorageAccountName).Value[0]
    }
}
